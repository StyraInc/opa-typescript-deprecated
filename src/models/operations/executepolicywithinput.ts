/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import * as components from "../../models/components";
import * as z from "zod";

/**
 * The input document
 */
export type ExecutePolicyWithInputRequestBody = {
    /**
     * Arbitrary JSON used within your policies by accessing `input`
     */
    input: components.Input;
};

export type ExecutePolicyWithInputRequest = {
    /**
     * The path separator is used to access values inside object and array documents. If the path indexes into an array, the server will attempt to convert the array index to an integer. If the path element cannot be converted to an integer, the server will respond with 404.
     */
    path?: string | undefined;
    /**
     * Indicates that the request body is gzip encoded
     */
    contentEncoding?: components.GzipContentEncoding | undefined;
    /**
     * Indicates the server should respond with a gzip encoded body. The server will send the compressed response only if its length is above `server.encoding.gzip.min_length` value. See the configuration section
     */
    acceptEncoding?: components.GzipAcceptEncoding | undefined;
    /**
     * If parameter is `true`, response will formatted for humans.
     */
    pretty?: boolean | undefined;
    /**
     * If parameter is true, response will include build/version info in addition to the result.
     */
    provenance?: boolean | undefined;
    /**
     * Return query explanation in addition to result.
     */
    explain?: components.Explain | undefined;
    /**
     * Return query performance metrics in addition to result.
     */
    metrics?: boolean | undefined;
    /**
     * Instrument query evaluation and return a superset of performance metrics in addition to result.
     */
    instrument?: boolean | undefined;
    /**
     * Treat built-in function call errors as fatal and return an error immediately.
     */
    strictBuiltinErrors?: boolean | undefined;
    /**
     * The input document
     */
    requestBody: ExecutePolicyWithInputRequestBody;
};

export type ExecutePolicyWithInputResponse = {
    httpMeta: components.HTTPMetadata;
    /**
     * Success.
     *
     * @remarks
     * The server also returns 200 if the path refers to an undefined document. In this case, the response will not contain a result property.
     *
     */
    successfulPolicyEvaluation?: components.SuccessfulPolicyEvaluation | undefined;
    headers: Record<string, Array<string>>;
};

/** @internal */
export namespace ExecutePolicyWithInputRequestBody$ {
    export type Inbound = {
        input: components.Input$.Inbound;
    };

    export const inboundSchema: z.ZodType<
        ExecutePolicyWithInputRequestBody,
        z.ZodTypeDef,
        Inbound
    > = z
        .object({
            input: components.Input$.inboundSchema,
        })
        .transform((v) => {
            return {
                input: v.input,
            };
        });

    export type Outbound = {
        input: components.Input$.Outbound;
    };

    export const outboundSchema: z.ZodType<
        Outbound,
        z.ZodTypeDef,
        ExecutePolicyWithInputRequestBody
    > = z
        .object({
            input: components.Input$.outboundSchema,
        })
        .transform((v) => {
            return {
                input: v.input,
            };
        });
}

/** @internal */
export namespace ExecutePolicyWithInputRequest$ {
    export type Inbound = {
        path?: string | undefined;
        "Content-Encoding"?: components.GzipContentEncoding | undefined;
        "Accept-Encoding"?: components.GzipAcceptEncoding | undefined;
        pretty?: boolean | undefined;
        provenance?: boolean | undefined;
        explain?: components.Explain | undefined;
        metrics?: boolean | undefined;
        instrument?: boolean | undefined;
        "strict-builtin-errors"?: boolean | undefined;
        RequestBody: ExecutePolicyWithInputRequestBody$.Inbound;
    };

    export const inboundSchema: z.ZodType<ExecutePolicyWithInputRequest, z.ZodTypeDef, Inbound> = z
        .object({
            path: z.string().default(""),
            "Content-Encoding": components.GzipContentEncoding$.optional(),
            "Accept-Encoding": components.GzipAcceptEncoding$.optional(),
            pretty: z.boolean().optional(),
            provenance: z.boolean().optional(),
            explain: components.Explain$.optional(),
            metrics: z.boolean().optional(),
            instrument: z.boolean().optional(),
            "strict-builtin-errors": z.boolean().optional(),
            RequestBody: z.lazy(() => ExecutePolicyWithInputRequestBody$.inboundSchema),
        })
        .transform((v) => {
            return {
                path: v.path,
                ...(v["Content-Encoding"] === undefined
                    ? null
                    : { contentEncoding: v["Content-Encoding"] }),
                ...(v["Accept-Encoding"] === undefined
                    ? null
                    : { acceptEncoding: v["Accept-Encoding"] }),
                ...(v.pretty === undefined ? null : { pretty: v.pretty }),
                ...(v.provenance === undefined ? null : { provenance: v.provenance }),
                ...(v.explain === undefined ? null : { explain: v.explain }),
                ...(v.metrics === undefined ? null : { metrics: v.metrics }),
                ...(v.instrument === undefined ? null : { instrument: v.instrument }),
                ...(v["strict-builtin-errors"] === undefined
                    ? null
                    : { strictBuiltinErrors: v["strict-builtin-errors"] }),
                requestBody: v.RequestBody,
            };
        });

    export type Outbound = {
        path: string;
        "Content-Encoding"?: components.GzipContentEncoding | undefined;
        "Accept-Encoding"?: components.GzipAcceptEncoding | undefined;
        pretty?: boolean | undefined;
        provenance?: boolean | undefined;
        explain?: components.Explain | undefined;
        metrics?: boolean | undefined;
        instrument?: boolean | undefined;
        "strict-builtin-errors"?: boolean | undefined;
        RequestBody: ExecutePolicyWithInputRequestBody$.Outbound;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, ExecutePolicyWithInputRequest> =
        z
            .object({
                path: z.string().default(""),
                contentEncoding: components.GzipContentEncoding$.optional(),
                acceptEncoding: components.GzipAcceptEncoding$.optional(),
                pretty: z.boolean().optional(),
                provenance: z.boolean().optional(),
                explain: components.Explain$.optional(),
                metrics: z.boolean().optional(),
                instrument: z.boolean().optional(),
                strictBuiltinErrors: z.boolean().optional(),
                requestBody: z.lazy(() => ExecutePolicyWithInputRequestBody$.outboundSchema),
            })
            .transform((v) => {
                return {
                    path: v.path,
                    ...(v.contentEncoding === undefined
                        ? null
                        : { "Content-Encoding": v.contentEncoding }),
                    ...(v.acceptEncoding === undefined
                        ? null
                        : { "Accept-Encoding": v.acceptEncoding }),
                    ...(v.pretty === undefined ? null : { pretty: v.pretty }),
                    ...(v.provenance === undefined ? null : { provenance: v.provenance }),
                    ...(v.explain === undefined ? null : { explain: v.explain }),
                    ...(v.metrics === undefined ? null : { metrics: v.metrics }),
                    ...(v.instrument === undefined ? null : { instrument: v.instrument }),
                    ...(v.strictBuiltinErrors === undefined
                        ? null
                        : { "strict-builtin-errors": v.strictBuiltinErrors }),
                    RequestBody: v.requestBody,
                };
            });
}

/** @internal */
export namespace ExecutePolicyWithInputResponse$ {
    export type Inbound = {
        HttpMeta: components.HTTPMetadata$.Inbound;
        SuccessfulPolicyEvaluation?: components.SuccessfulPolicyEvaluation$.Inbound | undefined;
        Headers: Record<string, Array<string>>;
    };

    export const inboundSchema: z.ZodType<ExecutePolicyWithInputResponse, z.ZodTypeDef, Inbound> = z
        .object({
            HttpMeta: components.HTTPMetadata$.inboundSchema,
            SuccessfulPolicyEvaluation:
                components.SuccessfulPolicyEvaluation$.inboundSchema.optional(),
            Headers: z.record(z.array(z.string())),
        })
        .transform((v) => {
            return {
                httpMeta: v.HttpMeta,
                ...(v.SuccessfulPolicyEvaluation === undefined
                    ? null
                    : { successfulPolicyEvaluation: v.SuccessfulPolicyEvaluation }),
                headers: v.Headers,
            };
        });

    export type Outbound = {
        HttpMeta: components.HTTPMetadata$.Outbound;
        SuccessfulPolicyEvaluation?: components.SuccessfulPolicyEvaluation$.Outbound | undefined;
        Headers: Record<string, Array<string>>;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, ExecutePolicyWithInputResponse> =
        z
            .object({
                httpMeta: components.HTTPMetadata$.outboundSchema,
                successfulPolicyEvaluation:
                    components.SuccessfulPolicyEvaluation$.outboundSchema.optional(),
                headers: z.record(z.array(z.string())),
            })
            .transform((v) => {
                return {
                    HttpMeta: v.httpMeta,
                    ...(v.successfulPolicyEvaluation === undefined
                        ? null
                        : { SuccessfulPolicyEvaluation: v.successfulPolicyEvaluation }),
                    Headers: v.headers,
                };
            });
}
